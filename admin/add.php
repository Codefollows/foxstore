<?php
	...
	if (!empty($_POST)) {
		...
		if ($type == "product") {
			...
			$data = array("cat_title" => cleanString($_POST['cat_title']));
			if (strlen($data['cat_title']) < 3 || strlen($data['cat_title']) > 20) {
				$error = array("Title must be between 3 and 20 characters");
			}
		}

		if (count($error) == 0) {
			$db->insert($type == "product" ? "products" : "categories", $data);
				...
				<div class="col-md-9 server-list">
					<div class="row">
						<div class="col-md-12">
							<div class="pull-right">
								<a href="products.php" class="btn btn-primary btn-xs">Go Back</a>
							</div>
							<h4 style="margin-top: 0;">Add <?php echo ucwords($type); ?></h4>
							<hr>
						</div>
					</div>
					...
					<div class="panel panel-default">
						<div class="panel-body">
							...
							?>
							<form action="add.php?type=<?php echo $type; ?>" method="post">
								<?php if ($type == "product"): ?>
									...
									<div class="form-group">
										<label>Price *</label>
										<input type="number" class="form-control" name="item_price" step="0.01" min="0.01">
									</div>
									<div class="form-group">
										<label>Discount *</label>
										<input type="number" class="form-control" name="item_discount" step="0.01" min="0.00">
									</div>
									<div class="form-group">
